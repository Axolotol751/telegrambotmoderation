...
def cmd_handler_names(cmd: str, bot_username: str):
    return [cmd, f"{cmd}@{bot_username}"]
...
async def on_startup(dispatcher):
    ...
    # --- BAN / UNBAN ---
    @dp.message_handler(commands=cmd_handler_names("ban", BOT_USERNAME))
    ...
    @dp.message_handler(commands=cmd_handler_names("unban", BOT_USERNAME))
    ...
    # --- MUTE / UNMUTE ---
    @dp.message_handler(commands=cmd_handler_names("mute", BOT_USERNAME))
    ...
    @dp.message_handler(commands=cmd_handler_names("unmute", BOT_USERNAME))
    ...
    # --- WARN / UNWARN / WARNS ---
    @dp.message_handler(commands=cmd_handler_names("warn", BOT_USERNAME))
    ...
    @dp.message_handler(commands=cmd_handler_names("unwarn", BOT_USERNAME))
    ...
    @dp.message_handler(commands=cmd_handler_names("warns", BOT_USERNAME))
...
dp.message_handler(commands=["start"])(  # остаётся вне on_startup, бот всегда активен
    lambda message: message.reply("Бот активен! Команды для админов: /ban, /unban, /mute, /unmute, /warn, /unwarn, /warns. Есть и пасхалки 😊")
)
